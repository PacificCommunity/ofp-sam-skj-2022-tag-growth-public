...
polygon(c(ages,rev(ages)),
        c(VB_output[1,],rev(VB_output[3,])), col=rgb(255,255,0, alpha=50, maxColorValue = 255), border=NA)
points(FL_cm~age_yrs, data=Leroy_oto, col="black", cex=0.8, pch=24, bg="white") # Overlay Leroy 2000 otolith data. 
...
jdat<-read.csv("tags_jp.csv") # 304 records
str(jdat)
...
jdat_comb<-data.frame(len=c(jdat$lenRel, jdat$lenRec, Leroy_oto$FL_cm))
jdat_comb$age<-c(jdat$rel.age,jdat$rec.age,Leroy_oto$age_yrs)
(vb <- vbFuns(param="Typical"))
...
# We use a high-quality subset of the tag-recapture PTTP dataset with records retained 
...
# Fit VB model to otolith and tag-recapture data
tt_comb<-data.frame(len=c(tt$lenRel, tt$lenRec, Leroy_oto$FL_cm))
tt_comb$age<-c(tt$rel.age, tt$rec.age, Leroy_oto$age_yrs)
(vb <- vbFuns(param="Typical"))
...
# Read in and explore screened data
spcdat<-read.csv("non-japanese-recaptures1.csv") # 59745 records
str(spcdat)
...
points(spcdat$lenRecFull~spcdat$rec.age, col="black") # recaptures
points(FL_cm~age_yrs, data=Leroy_oto, col="gold1") # Overlay Leroy 2000 otolith data. 

# Fit VB model to otolith and tag-recapture data
spcdat_comb<-data.frame(len=c(spcdat$lenRel, spcdat$lenRecFull, Leroy_oto$FL_cm))
spcdat_comb$age<-c(spcdat$rel.age,spcdat$rec.age,Leroy_oto$age_yrs)
(vb <- vbFuns(param="Typical"))
...
lines(spcdat.preds$fit~spcdat.preds$age, type="l", lwd=2, col="red")
...
# Plot of selected growth scenarios for 2022 SKJ assessment
ages<-seq(0.25,4,by=0.25) # redefine ages if need be
pdf("SKJ_growth_options_2022_Shimizu.pdf")
plot(vb(t=ages, Linf=93.6, K=0.43, t0=-0.49)~ages, type="n", xlim=c(0,4), ylim=c(0,115), # set plotting region based on # Tanabe et al. 2003 parameters
     xlab="Age (years)", xaxt="n", ylab="Fork length (cm)", las=1)
a <- 1:4
c = a[seq(1, length(a),1)]
...
# Overlay VB curve for Leroy 2000 otolith data and PTTP Shimizu combined
...
polygon(c(tt.preds$age,rev(tt.preds$age)),
        c(tt.preds$LCI,rev(tt.preds$UCI)), col=rgb(0,0,0, alpha=50, maxColorValue = 255), border=NA)
legend(x=2.5, y=35, legend=c("2022 DM", "WCPO meta", "Leroy 2000 & Shimizu", "Leroy 2000 data", "Tag release", "Tag recapture"), cex=0.95, 
       lty=c(1,1,1, NA, NA, NA), col=c("red2", "yellow", "black", "black", "black", "black"), lwd=c(4,2,2, NA, NA, NA), pch=c(NA, NA, NA,24,21,21), 
...
# or how recapture lengths and and date reliability are recorded. 
# That said, this data may be informative as they encompass release and recaptures in the northern
# region, not covered by PTTP/RTTP releases. This data also contains several useful datapoints with 
...
# Overlay VB curve for Leroy 2000 otolith data and PjdatP Shimizu combined
age <- seq(0.25,4,by=0.25) 
jdat.boot2 <- Boot(jdat.fit,f=predict_l) 
jdat.preds <- data.frame(age, predict_l(jdat.fit), confint(jdat.boot2))
names(jdat.preds) <- c("age","fit","LCI","UCI")
...
polygon(c(jdat.preds$age,rev(jdat.preds$age)),
        c(jdat.preds$LCI,rev(jdat.preds$UCI)), col=rgb(0,0,0, alpha=50, maxColorValue = 255), border=NA)
legend(x=2.5, y=35, legend=c("2022 DM", "WCPO meta", "Leroy 2000 & JPTP", "Leroy 2000 data", "Tag release", "Tag recapture"), cex=0.95, 
       lty=c(1,1,1, NA, NA, NA), col=c("red2", "yellow", "black", "black", "black", "black"), lwd=c(4,2,2, NA, NA, NA), pch=c(NA, NA, NA,24,21,21), 
       pt.bg=c(NA,NA,NA,"white","white", "black"), seg.len = 1.6, bty="n")
dev.off()
