...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_VECTOR(Aoto)        // age from otolith data
  DATA_VECTOR(Loto)        // length from otolith data
  ...
  vector<Type> Loto_pred(Noto);
  vector<Type> Loto_sd(Noto);
  vector<Type> Loto_MEsd(Noto);
  vector<Type> Loto_nll(Noto);

  vector<Type> curve(10*365+1);  // growth curve for age 0->10 (confint)
  ...
  {
    Loto_pred(i) = L0 + rmax*((log(exp(-K*a50)+1)-log(exp(K*(Aoto(i)-a50))+1))/K+Aoto(i));
    Loto_sd(i) = sd_a + sd_b * Loto_pred(i);
    Loto_MEsd(i) = sd_MEb * Loto_pred(i);
    Loto_nll(i) = -dnorm(Loto(i), Loto_pred(i), Loto_sd(i)+Loto_MEsd(i), true);
  }
  ans = sum(Lrel_nll) + sum(Lrec_nll) + sum(Loto_nll);
