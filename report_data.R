...
## After:  data_overview.pdf (report)
...
## otoliths
hist(oto$len, xlab="Length (cm)", xlim=c(30,80), col="white",
     main=paste0("otoliths (", nrow(oto), ")"))
hist(oto$age, xlab="Age (yr)", col="white",
     main=paste("Avg", round(mean(oto$age),1)))
plot(len~age, oto, xlab="Age (yr)", ylab="Length (cm)")
abline(lm(len~age, oto))
avg.oto <- coef(lm(len~age, oto))[[2]] / 365
main <- paste("Avg", formatC(mean(avg.oto), format="f", digits=3))
title(main=main)
plot(len~age, oto, subset=len>=40 & len<=60, ylim=c(40,60),
     xlab="Age (yr)", ylab="Length (cm)")
abline(lm(len~age, oto, subset=len>=40 & len<=60))
avg.oto <- coef(lm(len~age, oto, subset=len>=40 & len<=60))[[2]] / 365
main <- paste("Avg", formatC(mean(avg.oto), format="f", digits=3))
title(main=main)
dev.off()
