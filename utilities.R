...
plot.conv <- function(conv, x=c(-0.25,-0.15),
                      y=cumsum(c(0.9,rep(c(-0.06,-0.12),length=9))), cex=0.9)
{
  x <- rep(x, length=10)
  y <- rep(y, length=10)
  ...
  text( x[3],  y[3], pos=4, xpd=TRUE, cex=cex, "convergence")
  text( x[4],  y[4], pos=4, xpd=TRUE, cex=cex, conv$convergence)
  text( x[5],  y[5], pos=4, xpd=TRUE, cex=cex, "iterations")
  text( x[6],  y[6], pos=4, xpd=TRUE, cex=cex, conv$iterations)
  text( x[7],  y[7], pos=4, xpd=TRUE, cex=cex, "message")
  text( x[8],  y[8], pos=4, xpd=TRUE, cex=cex, conv$message)
  text( x[9],  y[9], pos=4, xpd=TRUE, cex=cex, "pdHess")
  text(x[10], y[10], pos=4, xpd=TRUE, cex=cex, conv$pdHess)
}

################################################################################
## Convert SPC geographic strings to numeric values

geo2num <- function(x)
{
  deg <- paste0(substring(x,1,n-7), ":", substring(x,n-6))
  deg2num(deg)
}
